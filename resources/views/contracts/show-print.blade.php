...
<head>
    ...
    <title>عقد ايجار - {{ $contract->code }}</title>
...
<body>
    <main>
        <div class="container">
            @if(!$is_cancelled && !$is_downpayment)
                <div class="buttons-wrap">
                    <a class="share-network-whatsapp" href="https://api.whatsapp.com/send?text=%D8%B9%D9%82%D8%AF%20%D8%A5%D9%8A%D8%AC%D8%A7%D8%B1%20{{ urlencode($share_link) }}" target="_blank">
        ...
        <div class="pdf-container">
            ...
            <div class="pdf {{ $is_cancelled ? 'blurred' : '' }} {{ $is_downpayment ? 'blurred-light downpayment' : '' }}" id="printMe">
                ...
                <div class="section-four">
                    ...
                    @else
                        ...
                        <p class="point">4. الإلتزام بإستخدام الوحدة السكنية لعائلة واحدة فقط تمثل عائلة المستأجر ، ويمنع منعاً باتاً الزيارة للمستأجر من الأفراد ، ويحق للإدارة إخلاء الوحدة في حال مغادرة العائلة ولا يحق للمستأجر استرجاع قيمة الأيجار بعد استلام الوحدة نهائياً ويتحمل المستأجر دفع غرامة بمبلغ 1000 ريال لصالح القطاع.</p>
                        ...
                        <tbody>
                            @foreach($contract->cars as $index => $car)
                                <tr>
                                    ...
                                    @if($index === 0)
                                        <td class="cus-ceil text-center" style="max-width: 140px" rowspan="{{ $contract->cars->count() }}">
                                            @if(!$is_downpayment)
                                                <img src="/images/paid.jpg" alt="" />
                                            @endif
                                        </td>
                                    @endif
                                </tr>
                    ...
                    <table class="table table-borderless mb-0">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="cus-ceil title-ceil cus-padding">المالك المفوض: <i>{{ $contract->user->name ?? '' }}</i></th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="cus-ceil cus-padding">
                                    <img src="https://api.qrserver.com/v1/create-qr-code/?size=250x250&data={{ urlencode($qr_code_link) }}" alt="QR Code" width="250" height="250" />
                                </td>
                                <td class="cus-ceil"></td>
                                <td class="cus-ceil">
                                    @if($is_reservation)
                                        <h6>ملاحظات عقود الحجوزات الإلكترونية:</h6>
                                        <h5>يلتزم المستأجر بدفع المبلغ المتبقي للمؤجر دون ادنى مسئولية على محرره</h5>
                                        <h3 style="color: red">{{ $remaining_payment }} ر.س</h3>
                                    @endif
                                </td>
                            </tr>
                        </tbody>
